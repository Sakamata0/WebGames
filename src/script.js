		...
		e.addEventListener("click", ()=>
		{
			if(!won && !lost)
			{
				...
				if(!e.classList.contains("flag"))
				{
					...
					else if(e.classList.contains("unrevealed"))
					{
						open(Number(e.id));
					}
				}
...
function open(block)
{
	...
	if(mineslist[block] == "0")
	{
		surroundings(block).forEach(e => {
			if(htmlBlock(e).classList.contains("unrevealed") && !htmlBlock(e).classList.contains("flag")) open(e);
		});
	}
	else
	{
...
function animation_flag_disappear(e)
{
